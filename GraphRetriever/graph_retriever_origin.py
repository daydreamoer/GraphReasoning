...
class GraphRetriever:
    ...
    def search_cell(self,query,topk=3,LLM_match_id_list=[],select_cell=False):
        ...
        # 如果 query 本身就是表格中某个cell的内容，则直接用精确匹配（EM）检索器找相关cell
        ...
        # 否则进入模糊匹配流程（稀疏检索 + 稠密检索）
        else:
            # 如果不指定 select_cell，就使用精确匹配（EM）做稀疏检索；否则，跳过稀疏检索，留空。
            if not select_cell:
            ...
            if not select_cell:
                ...
                # 如果稀疏检索结果不足 topk 个，就从稠密结果中筛选得分高的（大于某阈值），再调用EM检索获取cell并追加到结果里
                ...
                cell_topk,match_id_list,cell_topk_tuple = self.cell2Tuple(match_id_list,add_merged_cells=True)
            # select_cell == True 时的处理,和LLM的匹配结果融合，去重，并手动控制dense检索结果的组合方式
            else:
                dense_topk = topk
                temp_dense_topk,temp_dense_id_list = [],[]
                # 去除重复的 dense 匹配项：如果某个 dense 检索到的cell在稀疏结果或LLM结果中已经出现，则跳过，防止重复
        ...
        while error > 0:
            ...
            if json_match:
                ...
                try:
                    ...
                    print('LLM输出解析报错',select_id,e.__str__())
                    raise Exception('LLM输出解析报错 '+ select_id,e.__str__())
    ...
    def LLM_select_cells_from_table(self, query, topk=3,prompt_tamplate= 'LLM_select_cells'):
        ...
        # ]
        # 如果当前模型的名称中包含 'gemini'（说明使用的是 Google 的 Gemini 模型），则设置返回格式为 "application/json"。
        # 否则（例如使用的是 OpenAI GPT 模型或其他模型），则设置为 {"type": "json_schema"}，这是 OpenAI SDK 支持的一种用于结构化输出的格式。
