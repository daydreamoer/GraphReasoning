...
#增加json格式强制指令，并提供一个具体的 JSON 示例
ACTION_PROMPT_ENGLISH = """Based on the results of your previous Thought step {step}, generate the Action Step.

**IMPORTANT: You must strictly Output ONLY a JSON list.** Do not wrap it in markdown code blocks. Do not add explanations outside the JSON.
...
Example:
[{{"Function": {{"function_name": "GetSharedNeighbours", "parameters": [(0, 1, "Year"), (2, 0, "Revenue")]}}, "Explanation": "Checking intersection of Year and Revenue."}}]
...
# 新增repeated_action_attention，源代码中缺少
repeated_action_attention = """Warning: Step {step} repeats a previous action. Check if you need different query terms or if you already have enough data."""
...
PROMPT_TEMPLATE = {
    'hitab' :{
        ...
        'repeated_action_attention': repeated_action_attention,
    },
    'ait-qa': {
        'system_instruction': iterative_reasoning_system_instruction_english,
        'iterative_reasoning' :iterative_reasoning_english,
